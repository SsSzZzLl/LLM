    ...
    for p in sorted(corpus_dir.rglob("*")):
        if p.is_file() and p.suffix.lower() in {".txt", ".md", ".pdf"}:
            if p.suffix.lower() == ".pdf":
                text = extract_text_from_pdf(p, use_ocr=False) # Faster default
            else:
                text = read_text_file(p)
            source_id = str(p.relative_to(corpus_dir)).replace("\\", "/")
            all_chunks.extend(chunk_document(text, source_id, max_chars=max_chars, overlap_chars=overlap_chars))
    return all_chunks
